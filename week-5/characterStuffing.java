  ...
  public static void main(String args[]) throws IOException {
    ...
    while (true) {
      ...
      for (int i = 0; i < data.length(); i++) {
        ...
        // Stuff with 'E' if 'E' is found in the data to be sent else if (data.charAt(i) == 'E')
      ...
      // Send the data to the receiver dos.writeUTF(res); System.out.println("Seding Message	");
      if (dis.readUTF().equals("success")) System.out.println("Thanks for the Feedback Server!!");

      // End Messaging dos.writeUTF("bye"); break;

    }

    // Close all connections s.close();
    dis.close();
    dos.close();
  }
}
...
public class Byte_Stuffing {
  public static void main(String[] args) throws IOException {
    ...
    // Used to block until a client connects to the server Socket socket = servsock.accept();
    ...
    while (true) {
      ...
      // Used to read the data sent by client String res = dis.readUTF();
      System.out.println("Message Recevied...Successfully!!!");
      System.out.println("The Stuffed Message is : " + res);

      for (int i = 1; i < res.length() - 1; i++) {

        // If data contains a 'D' or 'F' do not unstuff it if (res.charAt(i) == 'D' || res.charAt(i) == 'F')
        out = out + res.charAt(i);

        // If data contains 'E' followed by 'E', de-stuff the former 'E' else if (res.charAt(i) == 'E' && res.charAt(i + 1) == 'E') {
        out = out + 'E';
        i++;
  ...
  // Closing all connections socket.close(); dis.close();
  dos.close();
}
}
